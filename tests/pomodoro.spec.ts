...
test.describe("ポモドーロタイマー", () => {
  ...
  test("セレクトボックスでタイマー値を変更できる", async ({ page }) => {
    // 集中時間を15分に変更
    await page.locator("#focusTime").selectOption("900");
    const focusTimerText = await page
      .getByText("集中する時間")
      .locator("..")
      .locator(".text-6xl")
    ...
    // 休憩時間を10分に変更
    await page.locator("#restTime").selectOption("600");
    const restTimerText = await page
      .getByText("休憩する時間")
      .locator("..")
      .locator(".text-6xl")
  ...
  test("スタートボタンでタイマーが動作する", async ({ page }) => {
    ...
    await expect(
      page.getByText("集中する時間").locator("..").locator(".text-6xl"),
    ).toHaveText("24:55");
  });
  ...
  test("集中時間が終わると自動的に休憩時間に切り替わる", async ({ page }) => {
    ...
    const focusTimer = page.getByText("集中する時間").locator("..");
    ...
    const restTimer = page.getByText("休憩する時間").locator("..");
    await expect(restTimer).toHaveClass(/border-green-500/);

    // 休憩時間が動き始めることを確認（5秒進める）
    await page.clock.runFor(5000);
    await expect(
      page.getByText("休憩する時間").locator("..").locator(".text-6xl"),
    ).toHaveText("04:55");
  });
});
